...
(contig_blast_res %>%
   ...
   left_join(glamr_samples_pg %>% collect() %>% dplyr::rename(sample = "SampleID")) %>%
   dplyr::rename(contig_taxonomy = "tax_name") %>% 
   filter(search_name == "guanitoxin") %>% 
   group_by(sample) %>% 
   mutate(n_gene_hits = n()) %>% 
  ggplot(aes(query_coverage, percent_id * 100, color = n_gene_hits, size = log10(tpm), tpm = tpm, contig = contig, gene = query, sample = sample, contig_taxonomy = contig_taxonomy, bin_info = bin_info, date = date, lat = lat, lon = lon)) +
  geom_point() + 
  scale_size_area(max_size = 5) +
  theme_bw() +
   scale_color_viridis_c(direction = -1) +
  facet_grid(query ~ search_name) +
   labs(y = "% ID", x = "Query % coverage", color = "Number of hits\nper sample")
) %>% plotly::ggplotly()
```
...
(contig_blast_res %>%
   ...
   left_join(glamr_samples_pg %>% collect() %>% dplyr::rename(sample = "SampleID")) %>%
   dplyr::rename(contig_taxonomy = "tax_name") %>% 
   filter(search_name == "skatole") %>% 
   group_by(sample) %>% 
   mutate(n_gene_hits = n()) %>% 
  ggplot(aes(query_coverage, percent_id * 100, color = n_gene_hits, size = log10(tpm), contig = contig, gene = query, sample = sample, contig_taxonomy = contig_taxonomy, bin_info = bin_info, date = date, lat = lat, lon = lon)) +
  geom_point() + 
  scale_size_area(max_size = 5) +
  theme_bw() +
   scale_color_viridis_c(direction = -1) +
